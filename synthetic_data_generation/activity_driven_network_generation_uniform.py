...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Run uniform activity temporal network.")
    parser.add_argument('--N', type=int, default=100, help='the number of nodes')
    parser.add_argument('--m', type=int, default=1, help='number of contacts in each step')
    parser.add_argument('--T', type=int, default=2000, help='time window of a temporal network')
...
def main(args):
    for iteration in range(1, 21):
        ...
        a_max = 1
        random_numbers = np.random.uniform(0, a_max, args.N)
        ...
        G_dataframe.to_csv(
            'uniform/' + 'uniform_acti_' + 'N' + str(args.N) + 'm' + str(args.m) + 'T' + str(args.T) + 'network' + 'iter' + str(
                iteration) + '.csv',
            index=None)
